...
if [ "$confirm" = "y" ]; then
    # Execute the command for each topic
    for topic in "${topic_array[@]}"
    do
        kafka-consumer-groups.sh --bootstrap-server $server --topic $topic --reset-offsets $reset_mode --execute
        echo "Reset executed for topic: $topic"
    done
    echo "All resets executed."
else
